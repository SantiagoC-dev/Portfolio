  ...
  return (
    <section className="py-20 lg:py-32 px-6 max-w-7xl mx-auto border-t border-gray-200 dark:border-gray-800">
      <div className="max-w-5xl mx-auto mb-12 lg:mb-16 relative">
        {/* Sticky solo en desktop para no robar espacio en móvil */}
      ...
      {layout === 'desktop' ? (
        ...
        // LAYOUT MÓVIL — marco de dispositivo fino, adaptado a 1, 2 o 3 columnas
        <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12 sm:gap-y-16 pb-10">
          {images.map((img, idx) => {
            // Lógica de desfase: en móvil no se desfasa, en tablet el 2do baja, en PC el del medio baja
            ...
            return (
              <motion.figure
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-50px" }}
                transition={{ duration: 0.8, delay: (idx % 3) * 0.15, ease: [0.215, 0.610, 0.355, 1.000] }}
                className={`w-full max-w-[240px] sm:max-w-[220px] mx-auto ${translateClass}`}
              >
      ...
      <section className="pt-32 sm:pt-40 md:pt-48 pb-16 md:pb-24 px-6 max-w-5xl mx-auto">
        <motion.div variants={staggerContainer} initial="hidden" animate="visible">
          ...
          <motion.button 
            ...
            className="flex items-center gap-2 md:gap-3 text-xs md:text-sm font-medium tracking-widest uppercase hover:text-black dark:hover:text-white transition-colors text-gray-400 dark:text-gray-500 mb-12 md:mb-20"
          >
            <svg className="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M10 19l-7-7m0 0l7-7m-7 7h18"/></svg>
            {t('caseStudy.ui.backButton')}
          </motion.button>

          <motion.span variants={fadeInUp} className="text-[10px] md:text-xs font-bold tracking-[0.2em] text-gray-400 dark:text-gray-500 uppercase mb-4 md:mb-6 block">
            {t('caseStudy.ui.headerLabel')}
          </motion.span>
          
          <motion.h1 variants={fadeInUp} className="text-5xl sm:text-6xl md:text-8xl font-serif mb-6 md:mb-8 tracking-tighter text-gray-900 dark:text-white leading-tight md:leading-none">
            {data.title}
          </motion.h1>
          
          <motion.p variants={fadeInUp} className="text-lg sm:text-xl md:text-3xl font-light text-gray-500 dark:text-gray-400 leading-relaxed max-w-3xl mb-12 md:mb-16">
            {data.subtitle}
          </motion.p>
          
          <motion.div variants={fadeInUp} className="grid grid-cols-2 sm:grid-cols-4 gap-6 md:gap-8 py-8 md:py-10 border-y border-gray-200 dark:border-gray-800">
            <div>
      ...
      <section className="py-20 lg:py-32 bg-white/40 dark:bg-gray-900/40 backdrop-blur-sm px-6 border-t border-gray-200 dark:border-gray-800 transition-colors duration-300">
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
          <div className="lg:col-span-4 relative">
            {/* Sticky desactivado en móvil para ahorrar espacio vertical */}
            <div className="lg:sticky lg:top-24">
              <h2 className="text-2xl md:text-3xl font-serif text-gray-900 dark:text-white tracking-tight">
            ...
            <div className="space-y-6 md:space-y-8">
              {data.problemPoints.map((point, index) => (
                <motion.div 
                  ...
                  className="flex flex-col sm:flex-row gap-1 sm:gap-4 lg:gap-6 border-l-2 border-gray-300 dark:border-gray-700 pl-5 hover:border-gray-800 dark:hover:border-gray-300 transition-colors duration-500"
                >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-8">
              {data.features.map((feature, i) => (
                <motion.div 
                  key={i}
                  whileHover={{ y: -5 }}
                  className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm p-6 md:p-8 rounded-2xl md:rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-lg dark:hover:shadow-gray-950/50 transition-all duration-500"
                >
